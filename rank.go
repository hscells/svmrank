...
// Learn trains a learning to rank model from the given features file and writes it to the specified model file.
func Learn(featureFile, modelFile string) {
	docs, _, totWords, totDoc := load(featureFile)
	learn(docs, totDoc, totWords, modelFile)
}

// Predict uses a model to predict the example and outputs the result to file.
